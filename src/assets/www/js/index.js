...
function dispara(){
	...
	window.setTimeout(function(){
		...
		if(c == 'off'){
			...
			$.post(getLink()+'/index.php', { token: str(), key: getDev(), device: 'mobile' }, function(data){
				console.log(data);	
...
function checking(){
	...
	window.setTimeout(function(){
		...
		if(c == 'off'){
			...
			$.post(getLink()+'/index.php', { checking: str(), key: getDev(), device: 'mobile' }, function(data){
				var dat = $.parseJSON(data);
				if(dat.update){
					window.location.href='update.html';
				}else{
...
function checkNumber(){
	var num = $('#device').attr('data-device');
	$.post(getLink()+'/index.php', {number: num, device: 'cel', key: getDev() }, function(data){
...
function directHome(){
	window.location.href='mobile.html#home';
}
...
function getHome(){
	...
	window.setTimeout(function(){ $('.pageApp').show().addClass('bounceInUp');}, 200);
	//window.setTimeout(function(){ $('.pageApp').removeClass('animated').removeClass('bounceInUp'); },1200);
...
function getRegister(){	
	...
	if(nome == ''){
		$('#l-quatro').addClass('rubberBand');
		$('#l-quatro input[type="text"]').addClass('err'); 
		window.setTimeout(function(){
			$('#l-quatro').removeClass('rubberBand'); 
			$('#l-quatro input[type="text"]').removeClass('err');
		},1000);
	}else if(gender == 1){
		$('#l-cinco').addClass('rubberBand');
		$('#l-cinco select').addClass('err'); 
		window.setTimeout(function(){
			$('#l-cinco').removeClass('rubberBand'); 
			$('#l-cinco select').removeClass('err');
		},1000);
	}else if(login == ''){
		$('#l-seis').addClass('rubberBand');
		$('#l-seis input[type="email"]').addClass('err'); 
		window.setTimeout(function(){
			$('#l-seis').removeClass('rubberBand'); 
			$('#l-seis input[type="email"]').removeClass('err');
		},1000);
	}else if(senha == ''){
		$('#l-sete').addClass('rubberBand');
		$('#l-sete input[type="password"]').addClass('err'); 
		window.setTimeout(function(){
			$('#l-sete').removeClass('rubberBand'); 
			$('#l-sete input[type="password"]').removeClass('err');
		},1000);
	}else{
		$('.sub3').attr('disabled', 'disabled').val('Aguarde');
		$.post( getLink()+'/gestao/register.php',  { login: login, pass: senha, name: nome, gender: gender, fb: fb, key: getDev() }, function(data){
			$('.sub3').removeAttr('disabled').val('Registrar');
			var dat = $.parseJSON(data);
			if(dat.return){
				var json = JSON.stringify(JSON.stringify(dat));
				$('#hashe').html(dat.token);
				DBinsert('2',json);
				redirectHome();
			}else if(dat.erro){
